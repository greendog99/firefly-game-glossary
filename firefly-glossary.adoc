...
Any time the "`Alliance Cruiser`" Nav Card is drawn, draw a new
*Alliance Alert Card*.
...
[.expansion]#Crime#
Any time the "`Alliance Cruiser`" Nav Card is drawn, draw a new
*Alliance Alert Card* and move the previous card to the bottom of its deck.
...
image::wanted-card.png[float=right,pdfwidth=28%]
...
your Work Action. Lose any repuation you had with the job owner and with Harken.
Kill a crew if you were working a job for Niska.

=== Morality

image::illegal-immoral.png[float=right,pdfwidth=33%]
...
[.expansion]#Pirates# Working on a Subjectively Moral job against a ship
...
[.expansion]#Flying# Whenever your Crew jumps ship (after being *Disgruntled*
twice or being hired by another player while disgruntled) roll a die. If the
result is lower than the number of Crew leaving, discard one *Ship Upgrade*
with the [.keyword]#SHUTTLE# keyword—they took off in your shuttle.
...
You can find Fightimage:skill-fight.png[fit=line],
Techimage:skill-tech.png[fit=line] and
Negotiateimage:skill-negotiate.png[fit=line] skill points on both *Crew* and
*Gear* Cards. Each symbol shown counts as one skill point when performing Skill
Tests.
...
image::amnon-duul.png[float=right,pdfwidth=62%]

When you are Solid, Contacts may buy Cargo and Contraband from you when dealing
with them, as listed in the top-right of their card, and can offer other
benefits. Contacts will not _sell_ you Cargo or Contraband unless their card
says "`Selling`".

If you receive a *Warrant* while working a Job, any Solid reputation
...
image::goal.png[float=right,pdfwidth=23%]
...
image::warrant.png[float=right,pdfwidth=25%]
...
You must kill a Crew if you receive a Warrant while working a job for Niska.
...
gear left on the ship may not be used during the Job.
...
If you’ve Loaded Cargo, Contraband, Passengers or Fugitives on to your
